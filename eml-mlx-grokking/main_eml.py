...
class NeuralNetwork:
    ...
    def train(self, train_data, val_data, epochs=5, shuffle=True):
        ...
        epoch_bar = tqdm(range(epochs), desc='Training (EML)', unit='epoch')
        for _ in epoch_bar:
            self.model.train()
            if shuffle:
...
def main(args):
    ...
    model = EMLGrokTransformer(**kwargs)
    ...
    ax.plot(np.array(net.train_acc_trace) * 100, 
            label='train (EML)', color='#1b9e77', lw=lw)
    ax.plot(np.array(net.val_acc_trace) * 100, 
            label='val (EML)', color='#d95f02', lw=lw)
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Accuracy (%)')
    ax.legend()
    fig.tight_layout()
    os.makedirs('media', exist_ok=True)
    fig.savefig('media/grokking_eml.png', dpi=300)
    print("Saved plot to media/grokking_eml.png")
